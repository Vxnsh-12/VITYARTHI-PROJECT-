...
A) Active Alerting: Immediate audio-visual alarms triggered by detected fatigue events.

B) Forensic Logging: Automated local storage of incident evidence (photos and logs) for post-event analysis.
...
C) Evidence Collection: An automated forensic system that captures timestamped evidence (photos and text logs) whenever a critical fatigue event occurs.

D) Heads-Up Display (HUD): A transparent video overlay that displays system status, low-light warnings, and tracking diagnostics directly on the video feed.
